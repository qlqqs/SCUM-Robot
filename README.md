...
本仓库以源码公开和可复现开发环境为目标，许可证为 MIT。

## 公开边界

- 仓库只公开源码、文档、配置模板。
- 不提交 `.env`、数据库、备份、`.venv`、`node_modules`、构建产物、`public` 生成内容、`zip`、`exe`。
- 如需发布二进制，请使用 GitHub Releases，不要放进源码仓库。

## 平台与风险说明

- `robot` 功能仅在 Windows 上完整支持，并且会控制本地窗口。
- `payment`、`backup` 属于管理员侧可选能力，默认不要求在本地开发时启用。
- 本次开源版本保留现有前后端集成方式与 REST API 形状，不做目录重命名和前后端重构。
